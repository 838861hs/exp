...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        for(let i = 0; i < CIRCLE_COUNT; ++i){
            ...
            // ランダムに色を変える
            let colorIndex = Math.floor(Math.random() * CIRCLE_COLORS.length);
            ...
            balls.push(new Ball(
                ...
                vy,
                speedPower,
                CIRCLE_COLORS[colorIndex]
            ));
        }

        // クリックイベントでカーソルに向かって進行方向を転換させる
        window.addEventListener('click', (eve) => {
            let targetX = eve.clientX;
            let targetY = eve.clientY;
            balls.map((ball) => {
                // ボールのその時点での位置
                let position = ball.getPosition();

                // カーソル位置とボールの位置を減算してベクトルとしてみなす
                let x = targetX - position.x;
                let y = targetY - position.y;

                // ベクトルを進行方向として使うので正規化する
